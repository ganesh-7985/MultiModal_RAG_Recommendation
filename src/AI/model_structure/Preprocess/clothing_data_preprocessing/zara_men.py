...
target_categories = {
    "BLAZERS", "HOODIES_SWEATSHIRTS", "JACKETS", "JEANS", "LINEN", "OVERSHIRTS", "POLO SHIRTS", 
    "SHIRTS", "SHOES", "SHORTS", "SWEATERS_CARDIGANS", "TROUSERS", "T-SHIRTS"
}

download_dir = os.path.join(os.path.expanduser("~"), "Downloads", "CLOTHES/MEN")
os.makedirs(download_dir, exist_ok=True)


csv_files = glob.glob(os.path.join(r"~\zara_data\Men", "*.csv"))
...
for file in csv_files:
    ...
    if ' product_images' in df.columns:
        for index, row in df.iterrows():
            ...
            try:
                image_list = ast.literal_eval(row[' product_images'])  
...
combined_df.to_csv("zara_men.csv", index=True, index_label="Index")